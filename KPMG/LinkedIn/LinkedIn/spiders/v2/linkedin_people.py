...
class Linkedinvoyagerpeoplesrp(scrapy.Spider):
	...
	def __init__(self, *args, **kwargs):
		...
		self.logins_dict = {'madhav': ['akramshaik541@gmail.com', 'Akramshaik@541']}
	...
	def parse_correct(self, response):
		...
		for datae in data_elements:
			...
			for ine in inner_elements:
				...
				image_link1 = mini_p.get('picture', {}).get('com.linkedin.common.VectorImage', {}).get('rootUrl', '')
				
				for imagepath in mini_p.get('picture', {}).get('com.linkedin.common.VectorImage', {}).get('artifacts', {}):
				    if imagepath.get('fileIdentifyingUrlPathSegment').startswith('100'):
					image_link2 = imagepath.get('fileIdentifyingUrlPathSegment')
				    else:
					continue
				if image_link1:
				    image_link = image_link1 + image_link2
				else:
				    image_link = ''
				...
				yield Request(positions, callback =  self.parse_main, headers = headers, meta = {"f_name":first_name, "l_name":last_name, "location":location, "profile_url":main_url, "image_url":image_link, "name":("%s%s%s" % (first_name, ' ', last_name)).strip()})
	...
	def parse_main(self, response):
		...
		values = [name, f_name, l_name, title, location, final_msg, company_name, exp_company_url, self.keyword, profile_url, image_url]
