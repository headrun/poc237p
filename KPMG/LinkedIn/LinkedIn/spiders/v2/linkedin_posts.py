...
class Linkedin_companies(scrapy.Spider):
	...
	data = [
	  ...
	  ('session_key', 'akramshaik541@gmail.com'),
	  ('session_password','Akramshaik@541'),
	  ('signin', 'Sign In'),
	  ('session_redirect', ''),
	  ('trk', 'hb_signin'),
    ...
    def parse_next(self, response):
        ...
        headers = {
            'cookie': 'li_at=%s;JSESSIONID="%s"' % (li_at_cookie, response.meta['csrf_token']),
                ...
                'authority': 'www.linkedin.com',
                'referer': 'https://www.linkedin.com/search/results/index/?keywords=pwc&origin=GLOBAL_SEARCH_HEADER',
        }
	api_url = "https://www.linkedin.com/voyager/api/search/cluster?count=6&guides=List(v-%3ECONTENT)&keywords=pwc&origin=SWITCH_SEARCH_VERTICAL&q=guided&start=0"
        yield Request(api_url, callback=self.parse_again, headers=headers, meta={"main_url":"https://www.linkedin.com/voyager/api/search/hits?guides=List(v->CONTENT)&keywords=pwc&origin=SWITCH_SEARCH_VERTICAL&q=guided", "headers":headers, "nav":"false"})
	...
	if response.meta.get('nav') == "false":
		for element in json_elements:
			...
			if profile:
				...
				if not content:
					content = element.get('hitInfo', {}).get('com.linkedin.voyager.feed.Update',{}).get('value',{}).get('com.linkedin.voyager.feed.ShareUpdate',{}).get('content',{}).get('com.linkedin.voyager.feed.ShareImage',{}).get('text',{}).get('values',[])

				if not content:
					content = element.get('hitInfo', {}).get('com.linkedin.voyager.feed.Update',{}).get('value',{}).get('com.linkedin.voyager.feed.ShareUpdate',{}).get('content',{}).get('com.linkedin.voyager.feed.ShareArticle',{}).get('text',{}).get('values',[])
				...
				information = ''
				for info in content:
				    information = information + info.get('value','')
		...
		for eachjso in json_elements:
			...
			if profile:
				...
				if not content:
					content = eachjso.get('hitInfo', {}).get('com.linkedin.voyager.feed.Update',{}).get('value',{}).get('com.linkedin.voyager.feed.ShareUpdate',{}).get('content',{}).get('com.linkedin.voyager.feed.ShareImage',{}).get('text',{}).get('values','')
				if not content:
					content = eachjso.get('hitInfo', {}).get('com.linkedin.voyager.feed.Update',{}).get('value',{}).get('com.linkedin.voyager.feed.ShareUpdate',{}).get('content',{}).get('com.linkedin.voyager.feed.ShareArticle',{}).get('text',{}).get('values','')

				information, likes, comments = '', '', ''
				for info in content:
					information = information + info.get('value','')
